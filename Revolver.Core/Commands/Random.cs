...
namespace Revolver.Core.Commands
{
  ...
  public class Random : BaseCommand
  {
    ...
    [FlagParameter("h")]
    [Description("Format the date as human friendly")]
    [Optional]
    public bool DateTimeFormatHuman { get; set; }
    ...
    public override CommandResult Run()
    {
      ...
      if(DateTimeFormatHuman && (!GenerateDates && !GenerateTimes))
        return new CommandResult(CommandStatus.Failure, "Parameter -h can only be used with -d or -t");
    ...
    {
      var parsedMinimum = DateTime.UtcNow.Date;
      ...
      var stringValue = DateUtil.ToIsoDate(value);

      if(DateTimeFormatHuman)
        stringValue = value.ToString();

      return new CommandResult(CommandStatus.Success, stringValue);
    }
